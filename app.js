// 기본 조항 데이터 백업 (file:// 로컬 열기 시 CORS 통과용)
const defaultData = [
  {
    "조": "제1조",
    "제목": "목적",
...
q.addEventListener('input', () => {
  ...
  const filtered = kw ? data.filter(a => a.본문.includes(kw)) : data;
  render(filtered);
});

// 데이터 로드 처리 (웹 서버 fetch 실패 시 local defaultData로 자동 전환)
fetch('조항데이터.json')
  .then(response => {
  ...
  .catch(() => {
    // 더블클릭(file://) 실행 시 CORS 방지로 인한 오류 발생 시 내장 데이터 사용
    data = defaultData;
    render(data);
  });